...
def gauss_seidel(A, b, x0=None, tol=1e-6, max_iter=100):
    ...
    for iter_count in range(max_iter):
        x_new = np.copy(x)
        for i in range(n):
            sum_1 = sum(A[i, j] * x_new[j] for j in range(i))  # Somatório de j = 0 até i-1
            sum_2 = sum(A[i, j] * x[j] for j in range(i + 1, n))  # Somatório de j = i+1 até n-1
            x_new[i] = (b[i] - sum_1 - sum_2) / A[i, i]  # Atualização de x_i
